...
The process often goes like this: Sample a finite number of (real!) 3-children families, count the number of girls in each family, and draw the frequency of each result. The *empirical* distribution. Since you have a limited budget, you usually have a lot less than a billion families, say 100, and the result might look like this:
...
This likelihood is the product of the probabilities of the samples of $G$ --- number of girls in 3-children families --- that we observed, assuming these families are independent:

$$
\begin{align}
Likelihood &= P(\text{2 girls in family 1}) \times P(\text{1 girl in family 2}) \times \cdots \times P(\text{0 girls in family 100}) \\
&= P(G = 2) \times P(G = 1) \times \cdots \times P(G = 0)
\end{align}
$$
...
And if instead of $G$, we looked at $X$ --- the height in centimeters of a random person --- which is normally distributed with some mean and variance we wish to estimate? Then the likelihood would be the product of the probability *densities* of the samples of $X$ that we observed, assuming these people are independent:

$$
Likelihood = f(X = 175) \times f(X = 180) \times \cdots \times f(X = 165)
$$
