...
Let's take this to the extreme. Look at the data again and tell me if a straight line is *the* best fit to the data. After all, some of the real data points are not on a straight line, so why not use a curve? Why not:
...
How do we know when a model is overfitting? Who can say if the parameters are necessary or not?

The famous statistician [George Box](https://en.wikipedia.org/wiki/George_E._P._Box) said: "All models are wrong, but some are useful." To me this says we should assume the model is wrong, that there is some error, noise, or *variability* in the data that we are not accounting for, and cannot account for. If we strive for the most complex model possible, it will start "eating" from that variability, fitting the noise, instead of learning the underlying pattern. And we should use a simpler model that is easier to understand and interpret.
