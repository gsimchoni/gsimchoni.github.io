...
In the case of my grandmother's model, the *compression* view of learning is even more appropriate. She started with 40 numbers (20 pairs of age and shoe-size), and compressed them to just two numbers, 18.5 and 1.5. From 40 to 2 -- that's 20 times fewer numbers, much more compact than the original data.

The same idea applies when we view the model *graphically*, as the red line she drew through the 20 dots:
...
When an unknown kid arrives, instead of asking for two numbers -- age and shoe-size -- to fully describe them, we only need to ask for their age, and then use the model to estimate their shoe-size. From 2 numbers to 1. Or, instead of needing a 2-D plot to place each kid, we just need to know how far along the red line they are. From 2-D to 1-D. Compression, either way.
