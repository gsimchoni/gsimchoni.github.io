...
$$
\text{model}_{\text{year}_i, \text{cow}_j} = \text{population mean} + \text{year effect}_i + \text{cow effect}_j
$$

That is, the milk production of a cow $j$ in a given year $i$ is the population mean, plus the effect of year $i$, plus the effect of the cow $j$. The population mean and the year effects are parameters, they are fixed. The cow effects are samples of a normally distributed random variable, $\mathcal{N}(0, \text{var}_{\text{cow}})$. Most importantly, how many parameters did Henderson have to estimate? The population mean, the three year effects, and the variance of the cow effects^[This isn't accurate. In order for this to work, we would need to either set one of the year effects to 0, or constrain the year effects to sum to 0. In addition, there is another variance of the error term, which I casually left out. But I think you get the idea.]. Doable.
