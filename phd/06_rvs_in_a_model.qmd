...
Above the $\text{intercept}_{\text{global}}$ and $\text{slope}_{\text{global}}$ remain *fixed* parameters for all kids. However, for each kid $j$, we have two "random parameters" --- samples from two different random variables: $\text{i-diff}_{\text{kid}_j}$ and $\text{s-diff}_{\text{kid}_j}$. And at each age, we have a random error term --- a sample from a third random variable: $\text{error}_{\text{age}, \text{kid}_j}$.^[Notice that the $\text{i-diff}_{\text{kid}_j}$, $\text{s-diff}_{\text{kid}_j}$ terms are sampled once for each kid, while the $\text{error}_{\text{age}, \text{kid}_j}$ term is sampled once for each age and kid. We'll get back to this later.]

So that's three random variables. And not just any random variables, but normal random variables, with a mean of 0 and some variance we need to learn: $\mathcal{N}(0, \text{var}_{\text{intercept}})$, $\mathcal{N}(0, \text{var}_{\text{slope}})$, and $\mathcal{N}(0, \text{var}_{\text{error}})$. For example, these distributions could look like this:
...
ax.plot(x, norm.pdf(x, 0, sigma_e), color=colors[2])
...
ax.annotate(r"$\mathcal{N}(0, \mathrm{var}_{\text{error}})$",
            xy=(-1.4, norm.pdf(-1.4, 0, sigma_e)), xytext=(-8.5, 0.5),
            color=colors[2], ha="left", va="center",
...
To make it concrete, imagine Kid 1 sampled (once!) an $\text{i-diff}_{\text{kid}_1}$ sample of +2.5 from the $\mathcal{N}(0, \text{var}_{\text{intercept}})$ distribution, and an $\text{s-diff}_{\text{kid}_1}$ sample of +0.5 from the $\mathcal{N}(0, \text{var}_{\text{slope}})$ distribution. Kid 2 sampled an $\text{i-diff}_{\text{kid}_2}$ sample of -1.0, and a $\text{s-diff}_{\text{kid}_2}$ sample of +0.2. And so on. And at each age or measurement, they sampled another small error term from the $\mathcal{N}(0, \text{var}_{\text{error}})$ distribution.^[So that even if we were to measure the same kid at the same age multiple times, we might get slightly different shoe sizes, due to measurement error.] Their shoe sizes would be calculated according to the above equations. That's the model.
