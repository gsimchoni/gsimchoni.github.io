...
Plan B: train one "global" default model that would predict the shoe size of any kid from any age, and 10 additional "personal" models, one for each kid in the dataset, having their parameters reflect the *difference* from the global model. We will call these, for short, $\text{i-diff}$ and $\text{s-diff}$ for intercept-diff and slope-diff, respectively.
...
So, a model with both *fixed* parameters and *random* variables samples? That's crazy. I bet you're dying to know how it works. Soon. Very soon. But first, a history lesson.