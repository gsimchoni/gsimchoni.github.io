...
Above the $\text{intercept}_{\text{global}}$ and $\text{slope}_{\text{global}}$ remain *fixed* parameters for all kids. However, for each kid $j$ at any age, we have three *random* samples from three different random variables: $\text{i-diff}_{\text{kid}_j}$, $\text{s-diff}_{\text{kid}_j}$, and $\text{error}_{\text{age}, \text{kid}_j}$.^[Notice that the $\text{i-diff}_{\text{kid}_j}$, $\text{s-diff}_{\text{kid}_j}$ terms are sampled once for each kid, while the $\text{error}_{\text{age}, \text{kid}_j}$ term is sampled once for each age and kid. We'll get back to this later.]

And not just any random variables, but normal random variables, with a mean of 0 and some variance we need to learn: $\mathcal{N}(0, \text{var}_{\text{intercept}})$, $\mathcal{N}(0, \text{var}_{\text{slope}})$, and $\mathcal{N}(0, \text{var}_{\text{error}})$. For example, these distributions could look like this:
...
Here, imagine Kid 1 sampled (once!) an $\text{i-diff}_{\text{kid}_1}$ of +2.5 from the $\mathcal{N}(0, \text{var}_{\text{intercept}})$ distribution, and a $\text{s-diff}_{\text{kid}_1}$ of +0.5 from the $\mathcal{N}(0, \text{var}_{\text{slope}})$ distribution. Kid 2 sampled an $\text{i-diff}_{\text{kid}_2}$ of -1.0, and a $\text{s-diff}_{\text{kid}_2}$ of +0.2. And so on. And at each age they sampled another error term from the $\mathcal{N}(0, \text{var}_{\text{error}})$ distribution.
